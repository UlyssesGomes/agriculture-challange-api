...
import { Controller, Get, Post, Put, Delete, Body, Param, Patch, NotFoundException, HttpCode } from '@nestjs/common';
...
export class PlantedCropController {
  ...
  @Post()
  @ApiBody({ type: PlantedCropDto, description: 'Json structure for planted crop object.' })
  @ApiOperation({ summary: 'Create a new planted crop' })
  @ApiResponse({ status: 201, description: 'Planted crop created successfully' })
  ...
  @ApiParam({ name: 'id', type: 'number', description: 'ID of the planted crop to find'})
  ...
  @ApiResponse({ status: 200, description: 'Planted crop updated successfully' })
  @ApiResponse({ status: 404, description: 'Planted crop not found' })
  @ApiParam({ name: 'id', type: 'number', description: 'ID of the planted crop to update'})
  async updatePlantedCrop(@Param('id') id: number, @Body() plantedCropData: any): Promise<PlantedCrop | undefined> {
    const plantedCropResponse = await this.plantedCropService.update(id, plantedCropData);
  ...
  @ApiResponse({ status: 204, description: 'Planted crop deleted successfully' })
  @ApiResponse({ status: 404, description: 'Planted crop not found' })
  @ApiParam({ name: 'id', type: 'number', description: 'ID of the planted crop to delete'})
